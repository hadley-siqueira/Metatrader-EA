...
double TopsBottoms::getTop(int i) {
   int k = top;
   int kk = top;
   int c = -1;
   ...
   return tbs[kk].candle.high;
}

double TopsBottoms::getBottom(int i) {
   int k = top;
   int kk = top;
   int c = -1;
   
   while (k >= 0 && c < i) {
      if (tbs[k].kind == BOTTOM) {
         ++c;
         kk = k;
      }
...
void drawBT() {
   ...
   for (int i = ArraySize(candles) - 1; i >= 0; --i) {
      if (isTop(i)) {
   ...
   int t = tbs.getTop(0);
   int tt = tbs.getTop(1);
   int b = tbs.getBottom(0);
   int bb = tbs.getBottom(1);
   
   if (tbs.trendingDown()) {
      //Comment("DOWN " + t + " " + tt + " " + b + " " + bb);
      Comment("DOWN");
   }
   
   if (tbs.trendingUp()) {
      //Comment("UP " + t + " " + tt + " " + b + " " + bb);
      Comment("UP");
   }
   
   if (tbs.lateral()) {
      //Comment("LATERAL " + t + " " + tt + " " + b + " " + bb);
      Comment("LATERAL");
   }
}
...
void trendStrategy() {   
   if (tbs.trendingUp()) {
      double top = tbs.getTop(0);
      double bottom = tbs.getBottom(0);
      double middle = round5((top + bottom) / 2, 5);
      double quarter = middle - round5((top + bottom) / 4, 5);
      double price = candles[0].close;
      
      if (near(price, middle)) {
         //Print("BUY: top: " + top + " bottom: " + bottom + " middle: " + middle + " price: " + price);
         trade.Buy(1, Symbol(), price); //, quarter, top);
         dir = BUY;
         g_gain = top;
         g_loss = quarter;
      }
   } else if (tbs.trendingDown()) {
      double top = tbs.getTop(0);
      double bottom = tbs.getBottom(0);
      double middle = round5((top + bottom) / 2, 5);
      double quarter = middle + round5((top + bottom) / 4, 5);
      ...
      if (near(price, middle)) {
         //Print("SELL: top: " + top + " bottom: " + bottom + " middle: " + middle + " price: " + price);
         trade.Sell(1, Symbol(), price);//, quarter, bottom);
         dir = SELL;
         g_gain = bottom;
         g_loss = quarter;
      }   
...
void OnTick() {
   ...
   if (hasPosition()) {
      if (candles[0].close == g_gain) {
         closeAllPositions();
      }
      
      if (candles[0].close == g_loss) {
         Print("loss");
         closeAllPositions();
      }
   } else {
